...
contract LunchVenueTestUpdated is LunchVenueUpdated {
    ...
    ////////////////////////////////////////////// Weakness 3 test seting Time_out/////////////////////////////////////////////
    function setTimeoutTest () public {
        Assert.ok( defineTimeOut(30) , "Faild to define Time-Out");
    }
    ...
    function setFriendFailure () public {
        try this.addFriend (acc4 , "Daniels") {
            Assert.ok(false , "Method execution should fail");
        } catch Error ( string memory reason ) {
            // Compare failure reason , check if it is as expected
            Assert.equal (reason , "Can only be executed by the manager", "Failed with unexpected reason");
    ...
    ///////////////////////////////// Weakness 2 startvote test////////////////////////////////

    /// Vote before beginVote failure ( acc1 )
    /// #sender: account-2
    function voteNotBeginFailure () public {
        try this.doVote (2){
            Assert.ok(false , "Method execution should fail");
        } catch Error ( string memory reason ) {
            // Compare failure reason , check if it is as expected
    ...
    function setStartVote () public {
        Assert.ok (beginVote(), "Failed to start Vote");
    }


    ///////////////////////////////// Weakness 4 disable contract test////////////////////////////////

    function disableContractTest() public {
        Assert.ok(endContract(),"Failed to end contract");

    }
    /// Vote after stop contract failure ( acc1 )
    function voteendContractFailure () public {
        try this.doVote (2){
            Assert.ok(false , "Method execution should fail");
        } catch Error ( string memory reason ) {
            // Compare failure reason , check if it is as expected
            Assert.equal (reason , "Can do it only when contract is vaild", "Failed with unexpected reason");
        } catch ( bytes memory /* lowLevelData */) {
            Assert.ok(false , "Failed unexpected");
        }
    }
    function enableContractTest() public {
        Assert.ok(startContract(),"Failed to start contract");
    }
...
///////////////////////////////// Weakness 1 Same account Vote test////////////////////////////////
    /// Vote as Bob ( acc1 )
    /// #sender: account-1
